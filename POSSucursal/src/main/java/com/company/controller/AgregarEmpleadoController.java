...
// Agrupar radio buttons
    ToggleGroup tg = new ToggleGroup(); // Generar la conexion
...
// Declaracion de la alerta
    ...
    @Override
    public void initialize(URL url, ResourceBundle rb) { // Cargar la hora
...
// Cargar la imagen
    ...
    @FXML
    private void insertToDB(ActionEvent event) { // Saber que en que nodo se acciona el evento
        ...
        if (evt.equals(this.btnRegistrarEmpleado)) {
            if (!this.txtContrasena.getText().isEmpty() && !this.txtContrasena2.getText().isEmpty() && !this.txtUsuario.getText().isEmpty()) {
                if (this.txtContrasena.getText().equals(this.txtContrasena2.getText())) { // Crear el query
